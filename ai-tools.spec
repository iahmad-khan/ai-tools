...
- [foreman.py] Fix status code when creating hostgroups.
- [foreman.py] Do not rename the IPMI interface in all cases.
- [ai-bs-vm] Fix default configuration file path (manpage)
...
- [ai-create-environment-metadata] Correct repository URL (Thanks PS).
- [ai-foreman] Fix regression configuring the logger.
- [ai-disownhost] Fix crash due to bad variable name (Thanks JI).
- [ai-add-param] Fix bad behaviour when re-adding a param (Thanks AG).
...
- [ai-bs-vm] Fail earlier if the AI repo is created empty.
- [ai-bs-vm] Allow subdomains.
